...
class GeneticSolver:
    def __init__(self):
        ...
        for i in range(1, self.tournament_size):
            probability = self.tournament_probabilities[i - 1] * (
                1.0 - self.tournament_winner_probability
            )
    ...
    def calculate_key_fitness(self, text):
        ...
        fitness = (bigram_fitness * self.bigram_weight) + (
            trigram_fitness * self.trigram_weight
        )
        return fitness
    ...
    def elitism(self, population, fitness):
        ...
        population_fitness = {
            k: v
            for k, v in sorted(population_fitness.items(), key=lambda item: item[1])
        }
